...
def permittivity(rho,T,W,sal,sitype,freq):
    ...
    #epsi[mask], epsii[mask] = drysnow(rho[mask],T[mask],freq,'Tinga-Voss-Blossey')    
    #epsi[mask], epsii[mask] = drysnow(rho[mask],T[mask],freq,'original')    
    epsi[mask], epsii[mask] = drysnow(rho[mask],T[mask],freq,'ImprovedBorn')    
    #epsi[mask], epsii[mask] = drysnow(rho[mask],T[mask],freq)
    ...
    epsi[mask], epsii[mask] = wetsnow(epsi[mask],epsii[mask],W[mask],rho[mask],freq,'original')
    ...
    epsi[mask], epsii[mask] = salineicewithairbubbles(sal[mask],T[mask],[],freq)
        
    # Multi-year sea ice: Sea ice with brine and air inclusions
    mask = (sitype==4)
    epsi[mask], epsii[mask] = salineicewithairbubbles(sal[mask],T[mask],rho[mask],freq)
    return epsi, epsii
...
def salinewater(T,sal,freq):
    ...
    Something in the implementation here is not correct. 
    
    [epsi,epsii] = salinewater(T,sal,freq)
        epsi_brine:   relative permittivity (real part of dielectric constant)
        epsii_brine:  dielectric loss factor (imaginary part of dielectric constant)
        T:            temperature [K]
        sal:          salinity [psu]
...
def randomneedles(epsi_host, epsii_host, epsi_inc, epsii_inc, volinc,method='PolderVanSanten'):
    """
    ...
    [epsi_mix,epsii_mix] = randomneedles(epsi_host, epsii_host, epsi_inc, epsii_inc, volinc)
    ...
    # Complex permittivities for pure ice and brine:
    ehost = epsi_host - epsii_host*1j
    einc = epsi_inc - epsii_inc*1j
    ...
    return epsi_mix, epsii_mix

def sphericalinclusions(epsi_host,epsii_host,epsi_inc,epsii_inc,volinc,method='ImprovedBorn'):
    """
...
def salineicewithairbubbles(sal,T,rho,freq):
    ...
    # Dielectric constant of pure ice:
    epsi_ice, epsii_ice = epureice(T,freq,'Hufford_Matzler')
    # Dielectric constant of brine:
    epsi_brine, epsii_brine = ebrine(T,freq,'StogrynDesargant')   
    ...
    epsi_spheres, epsii_spheres = sphericalinclusions(epsi_ice,epsii_ice,epsi_brine,epsii_brine,volbrine,'ImprovedBorn')
    
    # Assuming brine inclusions as random needles:
    #epsimix_needles, epsimix_needles = randomneedles(epsi_ice,epsii_ice,epsi_brine,epsii_brine,volbrine)
    
    # Use a mix of spheres and random needles:
    # epsi_mix = 1/2*(epsi_mix_needles+epsi_mix_spheres)
    # epsii_mix = 1/2*(epsii_mix_needles+epsii_mix_spheres)
    epsi_mix = epsi_spheres
    epsii_mix = epsii_spheres
    ...
    if len(rho) > 0:
        ...
        epsi_mix, epsii_mix = sphericalinclusions(epsi_mix,epsii_mix,eps_air,epsi_air,vol_air)
